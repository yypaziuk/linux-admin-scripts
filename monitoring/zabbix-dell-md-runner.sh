#!/bin/bash
# Dell MD Storage: SMcli runner for Zabbix.
# Arg 1: IP address of storage controller A
# Arg 2: IP address of storage controller B
# Collects full storageArray data to a temp file used by
# zabbix-dell-md-get.sh and zabbix-dell-md-discovery.sh.
...
if [[ -e "${zabbix_lock}" ]]; then
    ...
    start_time=$(date '+%s')
    "${smcli_bin}" "$1" "$2" -c "show storageArray;" -o "${conf_file}"
    finish_time=$(date '+%s')
